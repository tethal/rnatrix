...
The project is split into two library crates with clear separation of concerns:

**`natrix-runtime`** - Standalone runtime (no compiler dependencies)

- Runtime value representation (`Value`, `NxError`)
- Bytecode definitions and VM (Phase 2+)
- Can execute pre-compiled bytecode independently

**`natrix-compiler`** - Compiler toolchain (depends on `natrix-runtime`)

- Source representation (`Source`, `Span`, `SourceError`)
- Tokenizer, parser, AST
- AST interpreter (reference implementation, uses compiler infrastructure for error reporting)
- Bytecode compiler (AST → Bytecode, Phase 2+)

This architecture enables the bytecode VM to run without any compiler dependencies, while keeping the AST interpreter
close to the compiler infrastructure it relies on for error reporting.
...
Phase 2 includes a text-based bytecode assembler for testing and debugging.

**Rationale:**

- **Better test coverage** - Compiler only generates valid patterns; assembler lets us test VM edge cases, error
  handling, and malformed bytecode
- **Development workflow** - Can test VM independently before compiler exists
- **Debugging tool** - Useful for inspecting generated bytecode throughout all phases
- **Learning value** - Writing assembler reinforces understanding of bytecode format and instruction encoding

**Example assembly:**

```asm
// Constant pool
0: function "fibonacci" 1 fib_impl
1: int 2
...
Assembly syntax designed to work with existing tokenizer (colon for labels/constants, `//` comments, standard literals).
